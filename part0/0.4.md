...
sequenceDiagram
    ...
    activate server
    server-->>browser: [{ "content": "test note!", "date": "2023-7-14" }, ... ]
    deactivate server

    Note right of browser: The browser executes the callback function that renders the notes, including the newly added note with text "test note!"
```